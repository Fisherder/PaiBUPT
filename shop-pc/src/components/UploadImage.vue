<template>
    ...
    import { uploadImageApi } from "../img";
    import { ref } from "vue";
    ...
    let images = newImgRes.value.newImgUrl.filter((item) => item.url !=
    file.name);
    ...
    file.name = import.meta.env.VITE_BASE_API + res.data;
    //设置到返回子组件的数据里面
    newImgRes.value.newImgUrl.push({
    url:import.meta.env.VITE_BASE_API + res.data
    })
    emits('getImg',newImgRes.value)
    }
    };
