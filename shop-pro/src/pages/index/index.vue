...
<script setup>
	...
	//瀑布流
	const flowList = ref([])
	// 在 shop-pro/src/pages/index/index.vue 文件中
	// 首页轮播图
	const getBannerList = async () => {
	    ...
	    if (res && res.code == 200) {
	        ...
	        // 确保只显示有效的轮播图项
	        swipperList.value = res.data.filter(item => {
	            // 只有轮播图项包含有效商品才显示
	            return item.goods && item.goods.status === '0' && item.goods.sellStatus === '0' && item.goods.deleteStatus === '0'
	        });
	    }
	}
	//首页推荐
